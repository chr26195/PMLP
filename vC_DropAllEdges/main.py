...
for run in range(args.runs):
    ...
    # ---------------------------------------- self_loops are used in training -------------------------------------
    self_loops = torch.nonzero(torch.eye(n)).t()
    dataset.graph['self_loops'] = self_loops.to(device)
    # ---------------------------------------- self_loops are used in training -------------------------------------
    ...
    for epoch in range(args.epochs):
        ...
        out = model(dataset.graph['node_feat'], dataset.graph['self_loops'])
        ...
        result = evaluate(model, dataset, split_idx, eval_func, criterion, args, result=out)
