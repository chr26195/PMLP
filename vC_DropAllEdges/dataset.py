...
from torch_geometric.datasets import Planetoid, Amazon, Coauthor, Yelp, Flickr, Reddit2
...
class NCDataset(object):
    ...
    def get_idx_split(self, split_type='random', train_prop=.5, valid_prop=.25, tr_num_per_class=20, val_num_per_class=30):
        """
        ...
        if split_type == 'random_all':
            ...
            train_idx, valid_idx, test_idx = class_rand_splits(self.label, tr_num_per_class=tr_num_per_class, val_num_per_class=val_num_per_class)
            split_idx = {'train': train_idx,
                         'valid': valid_idx,
                         'test': test_idx}
        return split_idx
...
def load_dataset(data_dir, dataname, sub_dataname=''):
    ...
    elif dataname in ('yelp', 'flickr', 'reddit2'):
        dataset = load_inductive_dataset(data_dir, dataname)
    else:
...
def load_inductive_dataset(data_dir, name):
    transform = T.NormalizeFeatures()
    if name == 'yelp':
        torch_dataset = Yelp(root=f'{data_dir}yelp', transform=transform)
    elif name == 'flickr':
        torch_dataset = Flickr(root=f'{data_dir}flickr', transform=transform)
    elif name == 'reddit2':
        torch_dataset = Reddit2(root=f'{data_dir}reddit2', transform=transform)
