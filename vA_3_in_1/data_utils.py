...
def evaluate(model, dataset, split_idx, eval_func, criterion, args, result=None):
    if result is not None:
        ...
        # ------------------------------------------------------------------------------------- 
        out = model(dataset.graph['node_feat'], dataset.graph['edge_index'], use_conv= args.conv_te)
        out_va = model(dataset.graph['node_feat'], dataset.graph['edge_index' if args.trans else 'va_edge_index'], use_conv= args.conv_va)
        out_tr = model(dataset.graph['node_feat'], dataset.graph['edge_index' if args.trans else 'tr_edge_index'], use_conv = args.conv_tr)
        # ------------------------------------------------------------------------------------- 


    train_acc = eval_func(
        dataset.label[split_idx['train']], out_tr[split_idx['train']])
    valid_acc = eval_func(
        dataset.label[split_idx['valid']], out_va[split_idx['valid']])
    test_acc = eval_func(
...
@torch.no_grad()
def evaluate_whole_graph(model, datasets, eval_func):
    dataset_tr, dataset_val, dataset_te = datasets[0], datasets[1], datasets[2]
    model.eval()
    train_out = model(dataset_tr)
    train_acc = eval_func(dataset_tr.label, train_out)
    valid_out = model(dataset_val)
    valid_acc = eval_func(dataset_val.label, valid_out)
    test_out = model(dataset_te)
    test_acc = eval_func(dataset_te.label, test_out)

    return train_acc, valid_acc, test_acc, test_out
