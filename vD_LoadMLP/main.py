...
### Load model ###
model_mlp, model_gcn = parse_method(args, n, c, d, device)
...
for run in range(args.runs):
    ...
    model_mlp.reset_parameters()
    optimizer = torch.optim.Adam(model_mlp.parameters(),weight_decay=args.weight_decay, lr=args.lr)
    best_val = float('-inf')
    ...
    for epoch in range(args.epochs):
        ...
        out = model_mlp(dataset.graph['node_feat'])
        ...
        result = evaluate(model_mlp, dataset, split_idx, eval_func, criterion, args, result=out)
        ...
        if result[1] > best_val:
            ...
            # --------------------------------------- Save MLP Model ------------------------------------------
            torch.save(model_mlp.state_dict(), f"saved_models/mlp_{run}")
            # -------------------------------------------------------------------------------------------------
    ...
    ### Load pretrained MLP model as initialization for GNN ###
    model_gcn.load_state_dict(torch.load(f"saved_models/mlp_{run}"))
    result = evaluate(model_gcn, dataset, split_idx, eval_func, criterion, args)
    logger.add_result(run, result[:-1])
...
para_num = count_parameters(model_mlp)
...
with open(f"{filename}", 'a+') as write_obj:
    ...
    write_obj.write(f"method{args.train_prop if args.rand_split else ''}:{args.method}(trans:{args.trans}),\t lr:{args.lr},\t wd:{args.weight_decay},\t dpo:{args.dropout},\t l:{args.num_layers},\t o:{args.num_mps},\t hc:{args.hidden_channels},\t performance: {results.mean():.2f} $\pm$ {results.std():.2f},\t train time:{train_time: .6f},\t para_num:{para_num: .2f}\n")          